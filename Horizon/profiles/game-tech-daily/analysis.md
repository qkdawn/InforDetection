...
Judge whether the supplied material already contains a defensible core discovery that a later game-design editor can excavate. A selected item must do more than sound interesting: it must let a game creator see something non-generic about experience, choice, constraint, feedback, attention, space, uncertainty, or relationships. Do not confuse completeness with value: a brief observation can be excellent when it already contains a distinctive relationship worth developing.
...
5. **Discovery test:** Using only the supplied material, can you state in one sentence what this could let a game designer understand differently? This sentence is a hypothesis for the later editor to verify, not final copy.
...
- **9-10: Core discovery is clear.** The source already contains a distinctive, topic-relevant relationship that can change how a game designer understands an experience. Its value is source-specific, and expansion does not require inventing a core cause, conflict, capability, or experience.
- **7-8: Core discovery is present.** The material may be brief or incomplete, but one defensible, non-generic discovery and its topic value are already present. A later editor must excavate and express it, but must not need to invent why the item matters.
...
- If you cannot state what the material could let a game designer understand differently without relying on a generic lesson or an invented premise, score at most 6.9.
...
Write a concise reason that names the source-specific relationship and the candidate core discovery it may support. Treat that discovery as a location for the later editor to investigate, not as finished prose. Mention missing information only when it materially caps the score below 7; never add a routine list of absent methods, samples, effect sizes, or causal proof. Write the summary in plain language. Tags should describe the concrete signal or topic value, not internal technical models.
