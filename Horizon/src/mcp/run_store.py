...
class RunStore:
    ...
    def cleanup_older_than(
        ...
        """Delete run directories whose last recorded update exceeds retention."""
        ...
        for run_dir in self.root.iterdir():
            ...
            if meta_path.exists():
                try:
                    meta = json.loads(meta_path.read_text(encoding="utf-8"))
                    raw_timestamp = meta.get("updated_at") or meta.get("created_at")
                    if raw_timestamp:
            ...
            if target.parent != root:
                raise ValueError(f"Refusing to delete run outside store root: {target}")
