...
class HorizonOrchestrator:
    ...
    def _scoring_ai_config(self) -> AIConfig:
        """Return the AI configuration used by content scoring passes."""
        model = self.config.ai.scoring_model or self.config.ai.model
        return self.config.ai.model_copy(update={"model": model})
    ...
    async def fetch_all_sources(
        self, since: datetime, source_pool: str | None = None
    ) -> List[ContentItem]:
        ...
        Args:
            ...
            source_pool: RSS deployment pool. Defaults to collection config.
        ...
        selected_pool = source_pool or configured_pool
        include_unpooled_sources = selected_pool == "daily"
        async with httpx.AsyncClient(timeout=30.0) as client:
            tasks = []

            # GitHub sources
            if include_unpooled_sources and self.config.sources.github:
            ...
            # Hacker News
            if include_unpooled_sources and self.config.sources.hackernews.enabled:
            ...
            rss_sources = [
                ...
                if source.deployment_pool == selected_pool
            ]
            if rss_sources:
            ...
            # Reddit
            if include_unpooled_sources and self.config.sources.reddit.enabled:
            ...
            # Telegram
            if include_unpooled_sources and self.config.sources.telegram.enabled:
            ...
            if (
                include_unpooled_sources
                and self.config.sources.twitter
                and self.config.sources.twitter.enabled
            ):
            ...
            if (
                include_unpooled_sources
                and self.config.sources.openbb
                and self.config.sources.openbb.enabled
            ):
            ...
            if (
                include_unpooled_sources
                and self.config.sources.ossinsight
            ...
            if (
                include_unpooled_sources
                and self.config.sources.gdelt
                and self.config.sources.gdelt.enabled
            ):
            ...
            if (
                include_unpooled_sources
                and self.config.sources.google_news
