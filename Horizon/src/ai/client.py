...
class OpenAIClient(AIClient):
    ...
    _NO_RESPONSE_FORMAT_BASE_URL_HOSTS = {"mytokenpi.com"}
    _RAW_HTTP_BASE_URL_HOSTS = {"mytokenpi.com"}
    ...
    async def _do_request(
        ...
        *,
        ...
        if self._uses_raw_http():
            return await self._do_raw_http_request(request_kwargs)
        return await self.client.chat.completions.create(**request_kwargs)

    def _supports_response_format(self) -> bool:
        if self.provider in self._NO_RESPONSE_FORMAT:
            return False
        return not any(host in self.base_url for host in self._NO_RESPONSE_FORMAT_BASE_URL_HOSTS)

    def _uses_raw_http(self) -> bool:
        return any(host in self.base_url for host in self._RAW_HTTP_BASE_URL_HOSTS)

    async def _do_raw_http_request(self, request_kwargs: dict):
