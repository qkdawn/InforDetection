    ...
    *, requested: str, classified: str, method: str = "source_override"
    ...
    return ContentItem(
        ...
        processing=ProcessingResult(
            ...
            artifacts={"en": ContentArtifact(language="en", title="A release")},
        ),
    )
...
def test_explicit_profile_refresh_clears_incoherent_processing_state():
    ...
    profile = asyncio.run(ContentClassifier(SimpleNamespace(), PROFILES).resolve(item))
...
def test_failed_ai_classification_does_not_use_default_profile():
    async def complete(**kwargs):
        raise TimeoutError("classification timed out")

    item = make_processed_item(requested="auto", classified="other")
    item.processing = None

    with pytest.raises(RuntimeError, match="classification timed out"):
        asyncio.run(
            ContentClassifier(SimpleNamespace(complete=complete), PROFILES).resolve(
                item
            )
        )
