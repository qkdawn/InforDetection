...
def test_get_effective_config_redacts_expanded_query_and_header_secrets(
    ...
    config["sources"]["rss"][0]["url"] = (
        "https://example.com/feed?key=${FEED_TOKEN}&view=full"
    )
    config["sources"]["rss"][1]["url"] = (
        "https://${URL_USER}:${URL_PASSWORD}@example.com/private"
    )
    ...
    assert (
        result["config"]["sources"]["rss"][1]["url"]
        == "https://<redacted>@example.com/private"
    )
    assert (
        result["config"]["webhook"]["headers"]
...
def test_score_distribution_separates_candidates_from_rejects() -> None:
    items = [
        make_item("reject", 5.9),
        make_item("candidate", 6.4),
        make_item("selected", 7.2),
        make_item("exceptional", 9.1),
    ]

    assert HorizonPipelineService._score_distribution(items) == {
        "unscored": 0,
        "0-5": 1,
        "6-candidate": 1,
        "7-8": 1,
...
def test_fetch_items_includes_fetch_report_in_response_and_metadata(
    ...
    assert (
        service.run_store.load_meta(result["run_id"])["fetch_report"]
        == result["fetch_report"]
    )


def test_filter_items_uses_public_filtering_pipeline_api(
    tmp_path: Path, monkeypatch
) -> None:
    service = HorizonPipelineService(runs_root=tmp_path / "mcp-runs")
    monkeypatch.setattr(service, "_profiles", lambda ctx: PROFILES)
...
def test_filter_items_matches_native_filtering_pipeline(
    tmp_path: Path, monkeypatch
) -> None:
    service = HorizonPipelineService(runs_root=tmp_path / "mcp-runs")
    monkeypatch.setattr(service, "_profiles", lambda ctx: PROFILES)
    ...
    monkeypatch.setattr(
        ...
        storage,
        console,
        profiles: make_filtering_orchestrator(),
    )
    ...
    assert (
        mcp_result["balanced_digest_enabled"] == native_result.balanced_digest.enabled
    )
...
def test_run_pipeline_skips_enrichment_when_filter_is_empty(
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: (
            SimpleNamespace(
                config=SimpleNamespace(ai=SimpleNamespace(languages=["en", "zh"]))
            ),
            [],
            [],
        ),
...
def test_run_pipeline_uses_filtered_stage_when_all_enrichment_fails(
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: (
            SimpleNamespace(
                config=SimpleNamespace(ai=SimpleNamespace(languages=["zh"]))
            ),
            [],
            [],
        ),
...
def test_enrich_items_propagates_batch_failure(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        service, "_orchestrator", lambda loaded_ctx: FailingOrchestrator()
    )
...
def test_enrich_items_does_not_create_stage_when_all_items_fail(
    ...
    class FailingOrchestrator:
        async def enrich_items(self, items):  # type: ignore[no-untyped-def]
            return EnrichmentBatchResult(failures={item.id: "ValueError: empty story"})
