...
def test_fetch_all_sources_selects_only_requested_rss_pool(monkeypatch) -> None:
    orchestrator = make_orchestrator()
    rss_sources = [
        RSSSourceConfig(
            name="Daily",
            ...
            catalog_id="RSS-001",
            deployment_pool="daily",
        ),
        RSSSourceConfig(
            name="Weekly",
            url="https://example.com/weekly.xml",
            catalog_id="RSS-002",
            deployment_pool="weekly",
        ),
        RSSSourceConfig(
            name="Reserve",
            url="https://example.com/reserve.xml",
            catalog_id="RSS-003",
            deployment_pool="reserve",
        ),
    ]
    orchestrator.config = SimpleNamespace(  # type: ignore[assignment]
        collection=SimpleNamespace(default_source_pool="daily"),
        sources=make_sources(github=[object()], rss=rss_sources),
        extractors={},
    )
    captured = []

    def make_rss_scraper(sources, client, extractors):  # type: ignore[no-untyped-def]
        captured.extend(source.catalog_id for source in sources)
        return StubScraper()

    monkeypatch.setattr("src.orchestrator.RSSScraper", make_rss_scraper)

    items = asyncio.run(
        orchestrator.fetch_all_sources(SINCE, source_pool="weekly")
    )

    assert items == []
    assert captured == ["RSS-002"]
    assert orchestrator.last_fetch_report is not None
    assert [
