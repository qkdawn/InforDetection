...
def test_cleanup_removes_only_runs_older_than_retention(tmp_path: Path) -> None:
    store = RunStore(tmp_path)
    ...
    store.write_json(
        old_run,
        "meta.json",
        {"run_id": old_run, "updated_at": (now - timedelta(days=31)).isoformat()},
    )
    store.write_json(
        recent_run,
        "meta.json",
        {"run_id": recent_run, "updated_at": (now - timedelta(days=30)).isoformat()},
    )

    removed = store.cleanup_older_than(30, now=now)

    assert removed == 1
    assert not (tmp_path / old_run).exists()
    assert (tmp_path / recent_run).exists()
