...
Horizon 是一套面向游戏设计师、策划、XR 创作者和世界观研究者的自动化创意研究工作流。它持续聚合真实世界的信息，从高噪声素材中筛出值得继续发展的线索，再将事实、关系、系统问题和视觉机制组织成可直接阅读与转发的中文报告。

系统以 RSSHub 负责信息接入，以 Horizon API 执行分阶段分析，以 n8n 编排定时任务，并使用 Browserless 渲染报告卡片。一次完整日更从 190 个信息源开始，最终生成 22 张报告卡片并可发布到飞书。

![Horizon 从信息采集到报告发布的完整工作流](docs/assets/horizon-workflow.png)
